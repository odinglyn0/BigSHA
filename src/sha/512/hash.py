class SHA512:
    """SHA-512 hash implementation with optional salt support."""
    ...
    def __init__(self):
        """Init SHA-512 with initial hash values."""
        self.h = [
    ...
    def _pad_message(self, message):
        """Pad message according to SHA-512 specification."""
        msg_len = len(message)
        message += b'\x80'
    ...
    def _process_chunk(self, chunk):
        ...
        for i in range(80):
            t1 = (h + self._sigma1(e) + self._ch(e, f, g) + self.K[i] + w[i]) & 0xFFFFFFFFFFFFFFFF
    ...
    def hash_with_salt(self, message, salt):
        """
        Compute SHA-512 hash with salt (prepended to message).
