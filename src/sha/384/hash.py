class SHA384:
    """SHA-384 hash implementation with optional salt support."""
    ...
    def __init__(self):
        """Init SHA-384 with initial hash values."""
        self.h = [
    ...
    def _pad_message(self, message):
        """Pad message according to SHA-384 specification."""
        msg_len = len(message)
        message += b'\x80'
    ...
    def _process_chunk(self, chunk):
        ...
        for i in range(96):
            t1 = (f + self._sigma1(c) + self._ch(c, d, e) + self.K[i % len(self.K)] + w[i]) & 0xFFFFFFFFFFFFFFFF
            t2 = (self._sigma0(a) + self._maj(a, b, c)) & 0xFFFFFFFFFFFFFFFF
            f = e
            e = d
            d = c
            c = (b + t1) & 0xFFFFFFFFFFFFFFFF
            b = a
    ...
    def hash(self, message):
        """
        Compute SHA-384 hash of message.
    ...
    def hash_with_salt(self, message, salt):
        """
        Compute SHA-384 hash with salt (prepended to message).
